...
namespace FirstAPI.Services
{
    public class DoctorService : IDoctorService
    {
        ...
        public async Task<Doctor> GetDoctbyEmail(string email)
        {
            try
            {
                var AllDoctors = await _doctorRepository.GetAll();

                var result = AllDoctors.FirstOrDefault(u => u.Email == email);
                return result;
            }
            catch (Exception e)
            {
                throw new Exception("error finding doctor by email");
            }
